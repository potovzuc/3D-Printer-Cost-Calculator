import tkinter as tk
from tkinter import ttk
import base64
import os

def create_icon():
    icon_data = b''  # Buraya base64 kodlanmış logo.ico verisini ekle
    with open("temp_icon.ico", "wb") as icon_file:
        icon_file.write(base64.b64decode(icon_data))

def hesapla_maliyet(filament_gram, saat, cihaz_gucu_watt, filament_maliyet_kg):
    elektrik_maliyet_sanayi_kwh = 5.26774892
    elektrik_maliyet_ev_kwh = 2.9768688
    cihaz_gucu_kw = cihaz_gucu_watt / 1000
    
    filament_maliyeti = (filament_gram / 1000) * filament_maliyet_kg
    elektrik_maliyeti_sanayi = cihaz_gucu_kw * saat * elektrik_maliyet_sanayi_kwh
    elektrik_maliyeti_ev = cihaz_gucu_kw * saat * elektrik_maliyet_ev_kwh
    toplam_maliyet_sanayi = filament_maliyeti + elektrik_maliyeti_sanayi
    toplam_maliyet_ev = filament_maliyeti + elektrik_maliyeti_ev
    
    sonuc_text.set(f"Filament Maliyeti: {filament_maliyeti:.2f}₺\n"
                   f"Elektrik Maliyeti Sanayi: {elektrik_maliyeti_sanayi:.2f}₺\n"
                   f"Elektrik Maliyeti Ev: {elektrik_maliyeti_ev:.2f}₺\n"
                   f"Toplam Maliyet Sanayi: {toplam_maliyet_sanayi:.2f}₺\n"
                   f"Toplam Maliyet Ev: {toplam_maliyet_ev:.2f}₺")

def hesapla_ve_goster():
    try:
        filament = float(entry_filament.get())
        saat = float(entry_saat.get())
        cihaz_gucu_watt = float(entry_guc.get())
        filament_maliyet_kg = float(entry_filament_maliyet.get())
        hesapla_maliyet(filament, saat, cihaz_gucu_watt, filament_maliyet_kg)
    except ValueError:
        sonuc_text.set("Hata: Lütfen geçerli bir sayı girin!")

# Ana pencere
root = tk.Tk()
root.title("3D Yazıcı Maliyet Hesaplayıcı")
root.geometry("600x400")
root.configure(bg="#f0fff0")
create_icon()
root.iconbitmap("temp_icon.ico")

frame_main = tk.Frame(root, bg="#f0fff0")
frame_main.pack(fill=tk.BOTH, expand=True, padx=20, pady=20)

frame_left = tk.LabelFrame(frame_main, text="Girdi Bilgileri", bg="#f0fff0", font=("Arial", 12, "bold"), fg="#006400")
frame_left.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=10, pady=10)

frame_right = tk.LabelFrame(frame_main, text="Sonuçlar", bg="#f0fff0", font=("Arial", 12, "bold"), fg="#006400")
frame_right.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True, padx=10, pady=10)

style = ttk.Style()
style.configure("TButton", font=("Arial", 12), padding=5)

label_filament = ttk.Label(frame_left, text="Harcanılan Filament (gram):", background="#f0fff0", font=("Arial", 10))
label_filament.pack(pady=5)
entry_filament = ttk.Entry(frame_left, font=("Arial", 10))
entry_filament.pack(pady=5)

label_filament_maliyet = ttk.Label(frame_left, text="Filament Fiyatı (₺/kg):", background="#f0fff0", font=("Arial", 10))
label_filament_maliyet.pack(pady=5)
entry_filament_maliyet = ttk.Entry(frame_left, font=("Arial", 10))
entry_filament_maliyet.pack(pady=5)

label_saat = ttk.Label(frame_left, text="Kullanılan saat:", background="#f0fff0", font=("Arial", 10))
label_saat.pack(pady=5)
entry_saat = ttk.Entry(frame_left, font=("Arial", 10))
entry_saat.pack(pady=5)

label_guc = ttk.Label(frame_left, text="Cihaz Gücü (Watt):", background="#f0fff0", font=("Arial", 10))
label_guc.pack(pady=5)
entry_guc = ttk.Entry(frame_left, font=("Arial", 10))
entry_guc.pack(pady=5)

button_hesapla = ttk.Button(frame_left, text="Hesapla", command=hesapla_ve_goster)
button_hesapla.pack(pady=10)

sonuc_text = tk.StringVar()
label_sonuc = ttk.Label(frame_right, textvariable=sonuc_text, justify=tk.LEFT, anchor="w", padding=10, background="#f0fff0", font=("Arial", 10))
label_sonuc.pack(fill=tk.BOTH, expand=True)

root.mainloop()

# Uygulama kapandıktan sonra geçici ikon dosyasını silme
if os.path.exists("temp_icon.ico"):
    os.remove("temp_icon.ico")
